...
With the t-Test based analysis, we detected one out of four previously selected sure-shifters and all non-shifters. Additionally, MCM2_HUMAN - a belived non-shifter - was identified as an RBP with a significant shift. 
...
## Comapring Statistical analysis vs Clustering vs. Linear Regression
...
# Discussion
Our analysis allows us to detect potential RBPs that shift more than one fraction and accumulate in a single fraction after treatment with RNase. Our method only allowed us to detect absolute x-shifting values because we decided not to smoothen our data and did not apply a gaussian fit model. Therefore we were not able to detect RBPs that shift less then one absolute fractions. For example RBP2_HUMAN. RBP2_HUMAN is listed as an RBP in R-DeeP but only shifts for 0.1 fractions. Our method failed to detect this marginal shift. Another reason why we were not able to detect  this protein is that the chosen sample preparation method is not suited for this protein. A more fine density gradient could potentially result in a higher resolution able to detect the shift.

Furthermore we only apply our selection method on global maxima. This result in the inability to select proteins which only within local maxima fractions or where the global maxima is not the shifting fraction upon treatment with RNase which results in an TRUE/FALSE y-shift t-test. For example HDAC1_HUMAN (see R-DeeP Database). This results in a moderate precision (80.91 %) and a low accuracy for our analysis method of choice.

Our Clustering has the highest precision (96.87 % compared to 80.91 % and 60.21 %) of all applied models. Therefore Proteins which are located in Cluster B and C can be precisely identified as RBPs. Not proteins all proteins were located in these Clusters which displayed by our poor accuracy of 41.66 %. The accuracy of the clustering can be enhanced by choosing other clustering variables.

The trained linear regression model displayed a low precision of 60.21 % and also a low accuracy of 48.51 %. This caused by our poor $R^2$ of 0.2266 which results in severe under-fitting. Therefore the assignment of our 2081 proteins for model testing is not good and since the proteins are randomly selected it might happen that RBPS are over and under represented in the training cohort of our model.

All these aforementioned values do not add up to 100 % because only proteins that were listed in the provided data sets were cross-referenced. Therefore it might be that some potential RBPs are not detected and classified at all.



Regarding the in the beginning of our analysis selected and clasifed sure-shifters and non-shifters, Sin3A_HUMAN, HDAC1_HUMAN, HNRPU_HUMAN, RFC2_HUMAN and ASNS_HUMAN, MCM2_HUMAN, MCM3_HUMAN respectively all are listed as RBPs but obtained by a different method as published by Caudron-Herger *et al.*.Our method of analysis enabled us to detect one formerly as non-shifter classified RBP MCM2_HUMAN.

MCM2 serves as a S-Phase 
\newpage
