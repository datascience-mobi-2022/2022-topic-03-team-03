...
Next, our remaining data was analyzed per protein per fraction ctrl vs. RNase.
...
Here, the first three positive results are visualized.
...
The t-test often is positive and the graphs shows the "significant" difference in y-values right at the peak. However, as the graphs show for the first protein for example, the decrease in the RNAse peak fraction often is right next to the decrease in the ctrl fraction. This means, that sometimes visually the x-shift does not seem relevant enough to fully differentiate between a true shifter and a false positive. 

Nonetheless, in all three cases from above, the t-test for ctrl is actually negative.
...
So it might be useful to exclude all proteins, that do not have a "TRUE" in both the RNAse and ctrl maxima test. By doing so, we can hopefully reduce the amount of false positives because we did not smoothen our curves to match a gaussian fit for example. 

We quantified how many triplets in peak positions passed the t-test ("TRUE").

```{r Quantify t-test result}
# RNase
...
So following the t-test and F-test, these proteins were identified as non-RBP (every protein that failed either/both test at any point):

```{r}
# All proteins that entered analysis
All_potential_candidates <- row.names(HeLa_Ctrl_Norm)

# Identified as non_RBP
Non_RBP_t2 <- All_potential_candidates[!All_potential_candidates %in% row_2]
Non_RBP_t1 <- All_potential_candidates[!All_potential_candidates %in% row_1]
```
...
### Functions to compare our identified RBP with other publications
...
```{r True/False RBP}
# True RBP
True_RBP_t2 <- Find_true_RBP(row_2)
length(True_RBP_t2)

# False RBP
False_RBP_t2 <- Find_false_RBP(row_2)
length(False_RBP_t2)

# True non-RBP
True_non_RBP_t2 <- Find_true_non_RBP(Non_RBP_t2)
length(True_non_RBP_t2)

# False non-RBP
False_non_RBP_t2 <- Find_false_non_RBP(Non_RBP_t2)
length(False_non_RBP_t2)
```
...
Find_RBD <- function(potential_RBD) {
  RBD_stat <- RBD_stat_non_RBP[rownames(RBD_stat_non_RBP) %in% potential_RBD, ]     # Show only proteins from entered vector
  RBD_true_prot_df <- RBD_stat[RBD_stat$RBD_status == "RBD", ]                      # Show those with RBD status
  RBD_true_prot_names <- rownames(RBD_true_prot_df)                                 # Give names of RBD positives
...
On our data:
```{r}
False_RBP_true_RBD_t2 <- Find_RBD(False_RBP_t2)
length(False_RBP_true_RBD_t2)
```
...
```{r True/False RBP}
# True RBP
True_RBP_t1 <- Find_true_RBP(row_1)
length(True_RBP_t1)

# False RBP
False_RBP_t1 <- Find_false_RBP(row_1)
length(False_RBP_t1)

# True non-RBP
True_non_RBP_t1 <- Find_true_non_RBP(Non_RBP_t1)
length(True_non_RBP_t1)

# False non-RBP
False_non_RBP_t1 <- Find_false_non_RBP(Non_RBP_t1)
length(False_non_RBP_t1)
```

Perhaps some of our proteins that were non-shifters (although we classified them as shifters) are RNA dependent nonetheless:

```{r RBD status}
False_RBP_true_RBD_t1 <- Find_RBD(False_RBP_t1)
length(False_RBP_true_RBD_t1)
```

So 6 out of 120 proteins may at least be RNA-dependent although not RNA-binding.

```{r}
False_RBP_true_RBD_t1 == False_RBP_true_RBD_t2
```
...
```{r}
# >1 x-shift
( length(True_RBP_t1) / nrow(abs_max_both_x_y_1) ) * 100 # True positive quote
( length(False_RBP_t1) / nrow(abs_max_both_x_y_1) ) * 100 # False positive quote

( length(True_non_RBP_t1) / length(Non_RBP_t1) ) * 100 # True negative quote
( length(False_non_RBP_t1) / length(Non_RBP_t1) ) * 100 # False negative quote

# >2 x-shift
( length(True_RBP_t2) / nrow(abs_max_both_x_y_2) ) * 100 # True positive quote
( length(False_RBP_t2) / nrow(abs_max_both_x_y_2) ) * 100 # False positive quote

( length(True_non_RBP_t2) / length(Non_RBP_t2) ) * 100 # True negative quote
( length(False_non_RBP_t2) / length(Non_RBP_t2) ) * 100 # False negative quote
```
...
False_RBP_true_RBD_C == False_RBP_true_RBD_t1
```

The none of the RBD positive proteins from the t-test were identified again via clustering!
...
Approximately 96.9% of identified RBP in Cluster C were true RBP. 
...
False_RBP_true_RBD_B == False_RBP_true_RBD_t1
```
The none of the RBD positive proteins from the t-test were identified again via clustering!
...
Since the clustering and t-test show a very similar success rate, we were curious, if the identified RBPs were the same as well!
...
table(True_RBP_BC %in% True_RBP_t1)
```

It turns out, that most 217 out of 411 proteins in Cluster B+C were not present in identified proteins via t-test etc. 
...
So some proteins may at least be RNA-dependent although not RNA-binding.
...
It must also be mentioned, that not all proteins could be cross-referenced, since they appeared neither in the HS_RBPs nor the HS_non_RBPs data frame. 

```{r}
All_tested_reg <- c(True_RBP_reg, False_RBP_reg)
length(All_tested_reg)
length(RBP_regression)


Difference_reg <- RBP_regression[!RBP_regression %in% All_tested_reg]
Difference_reg
```