...
## Normalization
To rule out batch-to-batch effects and technical error, the protein amount per replicate ("Rep") and per fraction ("Frac") were additionally normalized with respect to each sample. The normalized results were then visualized by plotting the protein distribution in both samples.
...
Our methods detect potential RBPs that shift more than one fraction and accumulate in a single fraction after treatment with RNase. However, we detected absolute x-shifting values only because we decided against smoothening our data and did not apply a Gaussian fit model. Therefore, we were not able to detect RBPs that shift fewer than one absolute but more than zero fractions, for example, RBP2_HUMAN. On R-Deep, RBP2_HUMAN is listed as an RBP but only shifts 0.1 fractions. Our method failed to detect this marginal shift. 
...
The most similar results with 5259 shared classifications were returned by the comparison of the clustering and statistical tests. As a cross-reference, both methods seem to validate one another. On the other hand side, this shows that the clustering poses a viable short-cut compared to the extensive statistical tests. With the greated precision of 96.87 %, clustering may deliver more reliable results than the statistical tests in fact. However, the total amount of identified true RBPs is fewer in the clustering with 371 compared to 508. 
...
MCM3 is part of the MCM2-7 helicase complex which enables DNA replication during the S-Phase (Todorov *et al.*, 1994) of the cell cycle. The DNA replication complex is associated with topoisomerase I which interacts with RNA when replication stalls or strand breaks during the unwinding of the DNA strand occur (Takisawa *et al.*, 2000). Our data set was derived from HeLa cells synchronized in the interphase. The interphase consists of the $G_1$, $G_2$, and S-Phase (Malumbre & Barbacid, 2009). Therefore, it is plausible that expression levels of proteins playing a role in these phases such as MCM3 are elevated. RNA interaction would be present allowing for detection by our proposed methods.
