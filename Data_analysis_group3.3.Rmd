...
#ColSum
ColSum <- function(dataframe){
            dt <- dataframe
            lastcol <- dim(dataframe)[2]
            for(i in 1:lastcol){
              ColSum <- ColSums (dataframe[,i, drop= FALSE])
            # muss noch rausfinden, wie ich die Maximalwerte selektiere vermutlich über ne Seprate Funktion
              dt[,i] <- (Rep_max/ColSum_Rep)*dataframe[,i]
            }
            return(dt)
}
```
