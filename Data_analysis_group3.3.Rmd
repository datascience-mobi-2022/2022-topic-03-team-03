...
It turns out, that about half of the proteins shows a y-shift in maxima positions. We created a new dataframe containing those as well as new "trash cans" for those excluded based on failing the t-test.
...
The proteins that were filtered out by the graeter x-shift criterion are both claffisfied as RBPs by other studies as well as non-shifters. Since other work groups used the gaussian fit, the x-shift could be determined as 0.6 for example. In our case, we are working with integers that do not allow for partial fraction shifts as in 0.6. 
...
quotienten_HeLa <- quoti(HeLa_RNAse_mean, HeLa_Ctrl_mean)
q_HeLa_mat <- matrix(unlist(quotienten_HeLa), ncol=25, byrow=FALSE)
...
```{r}
maxi <- apply(q_HeLa_mat,1, max)
mini <- apply(q_HeLa_mat, 1, min)
mean <- rowMeans(q_HeLa_mat)
abs_mat_both_q <- cbind(abs_max_both, maxi,mini, mean)
dif <- abs_mat_both_q$maxi - abs_mat_both_q$mini
abs_mat_both_q <- cbind(abs_mat_both_q, dif)

clust <- c(maxi, x_shifts)
clus <- data.frame(maxi, x_shifts)
clus_left <- data.frame(maxi[left_shift], x_shifts[left_shift])
clus_no <- data.frame(maxi[no_shift], x_shifts[no_shift])
clus_right <- data.frame(maxi[right_shift], x_shifts[right_shift])
```

```{r silhouette method}
o <- data.frame(y_shifts2, q_mat_sums)
silhouette_score <- function(k){
  km <- kmeans(clus, centers = k, nstart=15)
  ss <- silhouette(km$cluster, dist(clus))
  mean(ss[, 3])
}
k <- 2:10
...
```{r cluster plot}
kmeans_fancy <- kmeans(scale(clus), 3, nstart = 25)
kmeans_fancy
# plot the clusters
fviz_cluster(kmeans_fancy, data = scale(clus), geom = c("point"),ellipse.type = "euclid")

kmeans_fancy <- kmeans(scale(clus_left), 2, nstart = 25)
kmeans_fancy
# plot the clusters
fviz_cluster(kmeans_fancy, data = scale(clus_left), geom = c("point"),ellipse.type = "euclid")

kmeans_fancy <- kmeans(scale(clus_right), 2, nstart = 25)
kmeans_fancy
# plot the clusters
fviz_cluster(kmeans_fancy, data = scale(clus_right), geom = c("point"),ellipse.type = "euclid")

kmeans_fancy <- kmeans(scale(clus_no), 2, nstart = 25)
kmeans_fancy
# plot the clusters
fviz_cluster(kmeans_fancy, data = scale(clus_no), geom = c("point"),ellipse.type = "euclid")
```

```{r}
sil <- silhouette(kmeans_fancy$cluster, dist(clus))
fviz_silhouette(sil)
```

```{r k means}
kmean1 <- kmeans(clus, centers = 3, iter.max = 25, nstart = 50)
kmean1

kmean1_table <- data.frame(kmean1$size, kmean1$centers)
kmean1_df <- data.frame(Cluster = kmean1$cluster, clus)
