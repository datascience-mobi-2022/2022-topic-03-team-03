...
```{r silhouette method}
h <- c(abs_max_both$y_shifts2, q_mat_sums)
o <- data.frame(y_shifts2, q_mat_sums)
silhouette_score <- function(k){
  km <- kmeans(o, centers = k, nstart=15)
  ss <- silhouette(km$cluster, dist(o))
  mean(ss[, 3])
}
k <- 2:10
...
```{r cluster plot}
l <- c(6,8)
kmean5 <- kmeans(o, centers = 3, iter.max = 25, nstart = 25)

kmeans_fancy <- kmeans(scale(o), 3, nstart = 25)
kmeans_fancy
# plot the clusters
fviz_cluster(kmeans_fancy, data = scale(o), geom = c("point"),ellipse.type = "euclid")
```
```{r}
sil <- silhouette(kmeans_fancy$cluster, dist(o))
fviz_silhouette(sil)
```

```{r k means}
kmean1 <- kmeans(o, centers = 3, iter.max = 25, nstart = 50)
kmean1

kmean1_table <- data.frame(kmean1$size, kmean1$centers)
kmean1_df <- data.frame(Cluster = kmean1$cluster, o)
...
#Here we are generating a list of all proteinn+```{r selecting highest quotient sum values}
#Here we checked the quotient sum value of each protein and selected only thos above 20 (random value), as those above 20 seem to contain many RBP's (comparison to RDeep database). 
...
likely_shifters
```ames, which have a quotient sum value higher than 20.
likely_shifters <- proteinnames[sums20_index]
likely_shifters
```
