...
```
```{r}
#rm(abs_max_both_F, abs_max_both_t, F_ctrl_norm, F_FALSE_ctrl, F_FALSE_RNase, F_NA_ctrl, F_NA_RNase, F_RNase_norm, p_value_matrix_t, t_ctrl_norm, t_FALSE_ctrl, t_FALSE_RNase, t_NA_ctrl, t_NA_RNase, t_RNase_norm, p_value_matrix, result)
...
```{r k means}
kmean1 <- kmeans(clust_new, centers = 4, iter.max = 25, nstart = 50)
kmean1

kmean1_table <- data.frame(kmean1$size, kmean1$centers)
kmean1_df <- data.frame(Cluster = kmean1$cluster, clust_new)
