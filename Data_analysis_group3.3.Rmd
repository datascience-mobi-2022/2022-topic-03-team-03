...
q_mat_sums <- rowSums(q_HeLa_mat, na.rm=TR
```
```{r}
 mini2 <- c()
...
samples.numbers.test <- proteins[-c(samples.numbers)]
...
for(x in 1:2081){shift_direction.regression[x] <- if(c[x] < 1) {'no shift'
}else  {' shift'}

}

compare <- cbind(c, x_abs[samples.numbers.test], shift_direction.regression, shift_direction[samples.numbers.test])


length(which(shift_direction.regression == "no shift"))
length(which(shift_direction[samples.numbers.test]== "no shift"))
