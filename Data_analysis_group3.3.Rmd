...
```{r True/False RBP}
load("/Users/carina/Desktop/HS_RBPs.RData")
load("/Users/carina/Desktop/HS_non_RBPs.RData")
...
RBD_stat_non_RBP_t_1 <- RBD_stat_non_RBP[rownames(RBD_stat_non_RBP_1) %in% False_RBP_names_1, ]
...
quotienten_HeLa_ctrl <- quoti(HeLa_Ctrl_mean, HeLa_RNAse_mean)
q_HeLa_mat <- matrix(unlist(quotienten_HeLa_RNAse), ncol=25, byrow=FALSE)
q_HeLa_mat_ctrl <- matrix(unlist(quotienten_HeLa_ctrl), ncol=25, byrow=FALSE)
q_mat_sums <- rowSums(q_HeLa_mat, na.rm=TRUE)
q_mat_sums_new <- q_mat_sums[prots]
sums20_index <- which(q_mat_sums>20)
sums20_value <- q_mat_sums[which(q_mat_sums>20)]
```

```{r}
maxi_RNAse <- apply(q_HeLa_mat,1, max)
maxi_ctrl <- apply(q_HeLa_mat_ctrl,1, max)
mini_RNAse <- apply(q_HeLa_mat, 1, min)
#mini_ctrl <- apply(q_HeLa_mat_ctrl,1, min)
mean <- rowMeans(q_HeLa_mat)
abs_mat_both_q <- cbind(abs_max_both, maxi_RNAse, mini_RNAse, mean)
dif <- abs_mat_both_q$maxi_RNAse - abs_mat_both_q$mini_RNAse
abs_mat_both_q <- cbind(abs_mat_both_q, dif)


```

```{r}
p_vals_prots <- rownames(abs_max_both) %in% rownames(p_value_df_t)
prots <-  c(which(p_vals_prots == TRUE))

abs_mat_both_q_new <- abs_mat_both_q[prots,]
fracs_RNAse_sign <- abs_mat_both_q_new$fracs_RNAse

p_vals_RNAse_max <- c(p_value_df_t[,fracs_RNAse_sign[x]])
p_vals_RNAse_max[is.na(p_vals_RNAse_max)] <- 0

maxi_new <- maxi_RNAse[prots]
maxi_ctrl_new <- maxi_ctrl[prots]

sums20_value_new <- sums20_value[prots]
```

```{r what do we cluster?}
o <- data.frame(abs_mat_both_q_new$y_shifts2, q_mat_sums_new)

m_c <- abs_mat_both_q$absolute_max_ctrl
m_R <- abs_mat_both_q$absolute_max_RNAse
j <- data.frame(maxi_RNAse, q_mat_sums)

clust <- data.frame(maxi_new, maxi_ctrl_new)

clus_new_idea <- data.frame(p_vals_RNAse_max, maxi_new)

clust_sum <- data.frame(sums20_value_new, maxi_new)
clust_sum[is.na(clust_sum)] <- 0
```


```{r silhouette method}
silhouette_score <- function(k){
  km <- kmeans(j, centers = k, nstart=15)
  ss <- silhouette(km$cluster, dist(j))
  mean(ss[, 3])
}
k <- 2:10
...
```{r cluster plot}
kmeans_fancy <- kmeans(scale(j), 3, nstart = 25)
kmeans_fancy
# plot the clusters
fviz_cluster(kmeans_fancy, data = scale(j), geom = c("point"),ellipse.type = "euclid")

shiftprotmaybe <- proteinnames[which(kmeans_fancy$cluster==2)]
...
```{r}
sil <- silhouette(kmeans_fancy$cluster, dist(j))
fviz_silhouette(sil)
```

```{r k means}
kmean1 <- kmeans(j, centers = 4, iter.max = 25, nstart = 50)
kmean1

kmean1_table <- data.frame(kmean1$size, kmean1$centers)
kmean1_df <- data.frame(Cluster = kmean1$cluster, j)
...
```{r ggplot shifters}
cluster1a <- as.matrix(abs_mat_both_q_new$fracs_ctrl[left_shift_new])
cluster1b <- as.matrix(abs_mat_both_q_new$fracs_RNAse[left_shift_new])
cluster2a <- as.matrix(abs_mat_both_q_new$fracs_ctrl[no_shift_new])
cluster2b <- as.matrix(abs_mat_both_q_new$fracs_RNAse[no_shift_new])
cluster3a <- as.matrix(abs_mat_both_q_new$fracs_ctrl[right_shift_new])
cluster3b <- as.matrix(abs_mat_both_q_new$fracs_RNAse[right_shift_new])

bla1 <- as.matrix(abs_mat_both_q_new$fracs_ctrl)
bla2 <- as.matrix(abs_mat_both_q_new$fracs_RNAse)
cols <- c(1,3)

p_all <- ggplot(abs_mat_both_q_new[,cols], aes(x = bla2, y=bla1), color = cyl) +
geom_point() +
  geom_density_2d(alpha = 0.5)
p0 <- p_all + labs(title = "All proteins")
p0

p_left <- ggplot(abs_mat_both_q_new[left_shift_new,cols], aes(x = cluster1a, y=cluster1b))+ 
  geom_point()+
    geom_density_2d(alpha = 0.5)
p1 <- p_left + labs(title = "Left shifters")
p1

p_no <- ggplot(abs_mat_both_q_new[no_shift_new,cols], aes(x = cluster2a, y=cluster2b)) +
  geom_point()+
  geom_density_2d(alpha = 0.5)
p2 <- p_no + labs(title = "Non-shifters")
p2

p_right <- ggplot(abs_mat_both_q_new[right_shift_new,cols], aes(x = cluster3a, y=cluster3b)) +
  geom_point()+
  geom_density_2d(alpha = 0.5)
...
```{r}
modell <- lm(sums20_value ~ abs_mat_both_q$x_shifts)
summary(modell)

plot(sums20_value, abs_mat_both_q$x_shifts, col ="blue", abline(modell), xlim = c(1, 100), ylim = c(1, 25))
```

```{r linear regression}
modell <- lm(abs_mat_both_q$x_shifts ~ abs_mat_both_q$maxi)
summary(modell)

plot(abs_mat_both_q$x_shifts, abs_mat_both_q$maxi, col ="blue", abline(modell))
