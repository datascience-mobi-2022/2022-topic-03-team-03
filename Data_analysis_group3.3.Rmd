...
```{r}
p_vals_prots <- rownames(p_value_df_t) %in% rownames(abs_max_both)
prots <-  which(p_vals_prots == TRUE)
sign_prots <- data.frame(mean_HeLa[ prots,], abs_max_both[prots,], p_value_df_t[ prots,])

i <- (1:7081)

p_vals_RNAse_max <- c()
p_vals_RNAse_max <- apply(p_value_df_t, 1, function(x){
  for (x in i){p_vals_RNAse_max[x] <- p_value_df_t[,abs_max_both$fracs_RNAse[x]]
  }
})
```
...
clus_new <- data.frame(abs_max_both_t)
```
