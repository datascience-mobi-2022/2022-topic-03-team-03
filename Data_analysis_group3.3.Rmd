...
else if(abs_max_both$x_shifts[x] < 0) {'left shift'}
else {'right shift'}
}
...
km.res <- kmeans(abs_max_both$x_shifts[!is.na(abs_max_both$x_shifts)], 4)
```
...
ggplot(abs_max_both[,cols], aes(x = bla2, y=bla1), color = cyl) +
  geom_point()

p_left <- ggplot(abs_max_both[left_shift,cols], aes(x = cluster1a, y=cluster1b, color = "blue")) +
  geom_point()
p1 <- p_left + labs(title = "Left shifters")
p1

p_no <- ggplot(abs_max_both[no_shift,cols], aes(x = cluster2a, y=cluster2b, color = "green")) +
  geom_point()
...
F_test_p_values = c()
for (i in 1:nrow(All_ctrl_norm)) {
  x <- as.numeric(All_ctrl_norm[i,])
  y <- as.numeric(All_RNAse_norm[i,])
...
F_test_p_values[1]
var.test(as.numeric(All_ctrl_norm[1,1:3]), as.numeric(All_RNAse_norm[1,1:3]), alternative = "two.sided")
```
...
```{r NaN}
F_test_p_values[21]
var.test(as.numeric(All_ctrl_norm[1,61:63]), as.numeric(All_RNAse_norm[1,61:63]), alternative = "two.sided")

# Checking on a NaN result
All_ctrl_norm[1,61:63]
All_RNAse_norm[1,61:63]
```
...
```{r FALSE}
F_test_p_values[3]
var.test(as.numeric(All_ctrl_norm[1,7:9]), as.numeric(All_RNAse_norm[1,7:9]), alternative = "two.sided")

# Checking on FALSE result
All_ctrl_norm[1,7:9]
All_RNAse_norm[1,7:9]
```
...
rownames(p_value_df) <-  row.names(All_ctrl_norm)
...
var.test(as.numeric(All_ctrl_norm[8, (1+3*k-3):(3*k)]), as.numeric(All_RNAse_norm[8, (1+3*k-3):(3*k)]), alternative = "two.sided")

# Measured protein content
k <- abs_max_both$fracs_RNAse[8] 

All_ctrl_norm[8, (1+3*k-3):(3*k)]
All_RNAse_norm[8, (1+3*k-3):(3*k)]

# Visually
compare_pre_post(row.names(All_ctrl_norm[8,]), 2000000, 110) # Protein row 8
...
var.test(as.numeric(All_ctrl_norm[41, (1+3*k-3):(3*k)]), as.numeric(All_RNAse_norm[41, (1+3*k-3):(3*k)]), alternative = "two.sided")

# Measured protein content
k <- abs_max_both$fracs_RNAse[41] 

All_ctrl_norm[41, (1+3*k-3):(3*k)]
All_RNAse_norm[41, (1+3*k-3):(3*k)]

# Visually
compare_pre_post(row.names(All_ctrl_norm[41,]), 2000000, 110) # Protein row 41
...
var.test(as.numeric(All_ctrl_norm[1, (1+3*k-3):(3*k)]), as.numeric(All_RNAse_norm[1, (1+3*k-3):(3*k)]), alternative = "two.sided")

# Measured protein content
k <- abs_max_both$fracs_RNAse[1] 

All_ctrl_norm[1, (1+3*k-3):(3*k)]
All_RNAse_norm[1, (1+3*k-3):(3*k)]

# Visually
compare_pre_post(row.names(All_ctrl_norm[1,]), 27000000, 30) # Protein row 1
...
var.test(as.numeric(All_ctrl_norm[42, (1+3*k-3):(3*k)]), as.numeric(All_RNAse_norm[42, (1+3*k-3):(3*k)]), alternative = "two.sided")

# Measured protein content
k <- abs_max_both$fracs_RNAse[42] 

All_ctrl_norm[42, (1+3*k-3):(3*k)]
All_RNAse_norm[42, (1+3*k-3):(3*k)]

# Visually
compare_pre_post(row.names(All_ctrl_norm[42,]), 2500000, 50) # Protein row 42
...
trash_F_FALSE <- c(row.names(F_FALSE_ctrl), row.names(F_FALSE_RNase))
...
trash_F_NA <- c(row.names(F_NA_ctrl), row.names(F_NA_RNase))
length(trash_F_NA)
```
...
```{r}
F_RNase_norm <- All_RNAse_norm[!rownames(abs_max_both) %in% trash_F_FALSE, ]
F_RNase_norm <- All_RNAse_norm[!rownames(abs_max_both) %in% trash_F_NA, ]

F_ctrl_norm <- All_ctrl_norm[!rownames(abs_max_both) %in% trash_F_FALSE, ]
F_ctrl_norm <- All_ctrl_norm[!rownames(abs_max_both) %in% trash_F_NA, ]
```
...
# Das sind erstmal wieder alle Fraktionen, also noch nicht auf die Maxima spezifisch hin. Das muss ich noch anpassen.
# Viel vom F-Test Code wird hier aber recyclet werden können
```
...
```{r p-values testing}
t_test_p_values_05 <- t_test_p_values < 0.05
```
...
>>>>>>> cf800c5c171e0bceb2a4f1fa8a06850b3c8c9b2b
