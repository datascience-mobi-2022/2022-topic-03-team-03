...
```{r True/False RBP}
load("/Users/Mirja/Desktop/HS_RBPs.RData")
load("/Users/Mirja/Desktop/HS_non_RBPs.RData")
...
#loading the clustering packages
library(corrplot)
library(cluster) 
library(factoextra)
```
...
```{r quotient function}
#Creating a function that works with the means of the fraction triplicate values. Here each RNase value of each protein and fraction is divided by the same protein and fraction control value. The quotient values are then saved in a new dataframe.
...
#Here we used the quotient function on our control and RNase mean values and saved the generated quotient values in a new matrix: q_HeLa_mat. Then we calculated the quotient sum value of each protein.
...
#Here we calculated different variables from our quotient matrix, to potentially use them for our clustering and regression analysis.
...
#Here we created a dataframe containing the two variables that generated the cluster with the best within cluster square distance.
cluster_data <- data.frame(maxi_RNAse, q_mat_sums)
```


```{r optimal number of clusters}
#We loaded the neccessary package and then used a function from that package that applies the silhouette method to our variables. This method gives us an optimal number of clusters of two.
library(factoextra)
fviz_nbclust(cluster_data, kmeans, method='silhouette')
```

```{r cluster plot}
#We are using kmeans clustering to cluster our data into three clusters. Although the silhouette plot gave us two as the optimal number of clusters, we found that three worked better and gave us better results. We then also plottet our clustered data.
...
#Here we did a silhouette plot with our clustered data, so we can see if our number of clusters is well chosen and if the data can be assigned well.
...
```{r plotting shifters}
#Here we plottet our previously defined left, right and non shifters using the ggplot2 package. This is for visualization purposes.
