...
abs_max_both_q <- cbind(abs_max_both, maxi_RNase, x_abs, mean,q_mat_sums)
```
...
ID <- c()
for(x in i){
ID[x] <- if (abs_max_both_q$x_shifts[x] == 0){abs_max_both_q$ID[x] =1 # x-shift = 0
} else if(abs_max_both_q$x_shifts[x] > 0){abs_max_both_q$ID[x] =2 # x-shift > 0
} else {abs_max_both_q$ID[x] =3} # x-shift < 0 
}
# Plot all proteins
control_fraction <- as.matrix(abs_max_both_q$fracs_ctrl)
RNase_fraction <- as.matrix(abs_max_both_q$fracs_RNase)
cols <- c(1,3)

colors <- c("right_shifters" = "green", "left_shifters" = "red", "non_shifters" = "black")

p_all <- ggplot(abs_max_both_q[,cols], aes(x = RNase_fraction, y=control_fraction, color = ID, show.legend = TRUE)) +
geom_point(color = ID) +
...
random.samples.training <- abs_max_both_q[samples.numbers.training,c(5,9,12)]
colnames(random.samples.training) <- c("x","max","sum")

random.samples.test <- abs_max_both_q[samples.numbers.test,c(5,9,12)]
