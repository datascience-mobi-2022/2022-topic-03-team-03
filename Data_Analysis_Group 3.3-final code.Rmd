...
We compared the true RBP detected via clustering and statistical tests.
```{r}
...
# How many cluster true RBP also in statistical test true RBP?
table(True_RBP_BC %in% True_RBP_t)

# How many cluster RBP also in statistical test RBP?
length(Cluster_BC)
length(RBP_t)
table(Cluster_BC %in% RBP_t)
```
About half of true RBP identified via clustering also identified via statistical tests.

>>>>>>> 56d8ea33f34ec42b74086cd39d9bca0cef455107
...
# True non-RBP
True_non_RBP_overlap <- Find_true_non_RBP(overlap.potential.non.RBP.id.clus)
length(True_non_RBP_overlap)

# False non-RBP
False_non_RBP_overlap <- Find_false_non_RBP(overlap.potential.non.RBP.id.clus)
length(False_non_RBP_overlap)
```
=======
table(non_shifters %in% RBP_regression) # non-shifters in shifters?
```


```{r}
compare_pre_post("MCM3_HUMAN", 40000000, 30)
```
