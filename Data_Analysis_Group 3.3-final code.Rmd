...
```{r split reps}
#Sorting the data set to select replicates 
# Control
...
#select proteins to train the model
proteins <- c(1:7081)
...
plot(random.samples.training$x,random.samples.training$max, col ="blue", abline(modell), ylim = c(0,15), xlab = "x-shift", ylab = " Maximum of quotients")
plot(random.samples.training$x,random.samples.training$sum, col ="blue", abline(modell), ylim = c(0,15), xlab = "x-shift", ylab = "Sums of quotients"
     
```

## test regression 
```{r test the regression}
# use model to predict x-shifts for the test set
predicted.x_shifts <- predict(modell, newdata = random.samples.test)

# analyse predicted values 
i <- c(1:2081)
shift_direction.regression <- c()
for(x in i){shift_direction.regression[x] <- if(predicted.x_shifts[x] < 1) {'no shift'
}else  {'shift'}
}

# identified x-shifts with the code
x_shifters <- x_shifts[samples.numbers.test]
shift_direction.code <- c()
for(x in i){shift_direction.code[x] <- if(x_shifters[x] == 0) {'no shift'
}else  {'shift'}
}

# compare code and regression results 
compare <- cbind(predicted.x_shifts, x_abs[samples.numbers.test], shift_direction.regression, shift_direction.code)
